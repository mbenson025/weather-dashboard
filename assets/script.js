  ...
  if (cityInput != '') {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        $('#dailyTitle').html(
          `${dailyTitle} <img src="https://openweathermap.org/img/wn/${icon}.png"/>`
        );
        ...
        $.ajax({
          ...
          success: function (data) {
            ...
            $('#dailyConditions').append(
              `<li>UV Index: <span id="uvColor">${propUV}</span></li>`
            );
            // console.log(propUV);
            ...
            if (propUV <= 3) {
              $('#uvColor')
                .css('background-color', '#008F39')
                .attr('class', 'badge');
            } else if (propUV <= 5) {
              $('#uvColor')
                .css('background-color', '#FEC901')
                .attr('class', 'badge');
            } else if (propUV <= 7) {
              $('#uvColor')
                .css('background-color', '#e36530')
                .attr('class', 'badge');
            } else if (propUV <= 10) {
              $('#uvColor')
                .css('background-color', '#cc3129')
                .attr('class', 'badge');
            } else {
              $('#uvColor')
                .css('background-color', '#a76ebb')
                .attr('class', 'badge');
            }
